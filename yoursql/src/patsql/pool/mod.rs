...
pub fn build_basic_pool_with_tcp_test(
    ...
    connection_timeout: u64) -> Result<Pool, String> {
        ...
        let trial_run: Result<TcpStream, ConnectionError> = tcp_connect_with_timeout(socket, five_seconds);

        if trial_run.is_err() == false {
...
// JSON SHOULD BE BUILT WITH THE FOLLOWING OPTIONS:
// hostname
// port
// db
// user
